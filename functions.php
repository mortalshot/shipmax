...
/**
 * shipmax functions and definitions
 *
...
if (!function_exists('spm_setup')) :
	...
	function spm_setup()
	{
		 ...
		 * If you're building a theme based on shipmax, use a find and replace
		 * to change 'spm' to the name of your theme in all the template files.
		 */
		load_theme_textdomain('spm', get_template_directory() . '/languages');
		...
		register_nav_menus(
			array(
				'header' => esc_html__('Header menu', 'spm'),
			)
		);
...
add_action('after_setup_theme', 'spm_setup');
 ...
 */
function spm_content_width()
{
	$GLOBALS['content_width'] = apply_filters('spm_content_width', 640);
}
add_action('after_setup_theme', 'spm_content_width', 0);

/**
 * Enqueue scripts and styles.
 */
function spm_scripts()
{
	wp_enqueue_style('spm-style', get_stylesheet_uri(), array(), _S_VERSION);
	wp_enqueue_style('spm-stylesheet', get_template_directory_uri() . '/dist/css/style.min.css', array(), _S_VERSION);

	wp_enqueue_script('spm-custom-js', get_template_directory_uri() . '/dist/js/app.min.js', '', _S_VERSION, true);
}
add_action('wp_enqueue_scripts', 'spm_scripts');
